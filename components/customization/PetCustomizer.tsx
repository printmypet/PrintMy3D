...
export const PetCustomizer: React.FC<PetCustomizerProps> = ({ value, onChange, partsColors, availableTextures, hidePetName = false, partImages, colorLabels, hideTexture = false, maxColors = 3 }) => {
  const colors = {
  ...
  return (
    <div className="space-y-5">
      ...
      {maxColors === 1 && (
        <ColorPicker label="Cor" colors={allColors} selected={value.baseColor} onSelect={c => onChange({ ...value, baseColor: c, ballColor: c, topColor: c })} />
      )}
      ...
      {maxColors === 2 && (
        <>
          ...
          <ColorPicker label={colorLabels?.top ?? "Cor Superior"} colors={colors.top} selected={value.topColor} onSelect={c => onChange({ ...value, topColor: c })} imageUrl={partImages?.top} texture={value.textureValue || undefined} />
        </>
      )}
